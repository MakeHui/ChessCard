cc.Class({
    ...
    properties: {
        ...
        jiaofenNodes: [cc.Node],    // 叫分按钮
        qiangdizhuNodes: [cc.Node], // 抢地主按钮
        chupaiNodes: [cc.Node],     // 出牌按钮
        readyNode: cc.Node, // 准备按钮
        clockNodes: [cc.Node],
    },
    onLoad() {
        this._Cache = {};
        ...
        if (window.Global.Config.tempCache) {
            ...
            window.Global.NetworkManager.onopen = () => {
                self._hideWaitPanel();
                window.Global.NetworkManager.sendSocketMessage(window.DDZ.NetworkConfig.WebSocket.EnterRoom, { roomId: self._Cache.roomId });
                window.Global.NetworkManager.sendSocketMessage(window.DDZ.NetworkConfig.WebSocket.Ready);
    ...
    onEnterRoomMessage(data) {
        ...
        this._setRoomInfo(data.kwargs, 1);
    ...
    onExitRoomMessage(data) {
        if (data.playerUuid == this._userInfo.playerUuid) {
            ...
            cc.director.loadScene('DDZLobby');
            return;
        }
    ...
    closeOnClick() {
        window.Global.SoundEffect.playEffect(window.Global.Config.audioUrl.effect.buttonClick);
        if (this._Cache.playerList.length !== 3) {
            window.Global.NetworkManager.sendSocketMessage(window.DDZ.NetworkConfig.WebSocket.ExitRoom);
        }
        else {
    ...
    _setRoomInfo(info, currentRound) {
        // 游戏玩法
        var playTypes = window.DDZ.Config.playTypes[info.game_uuid];
        var options = `0x${info.options.toString(16)}`;
        var num = info.options & 0xb10;
        num = (num == 0xb10) ? 0xb10 : 0xb1;

        this.roomInfo[3].string = `${playTypes.playType[num]}玩法\n炸弹上限: ${playTypes.options[options ^ num]}`;
        this.roomInfo[1].string = `房间号: ${this._Cache.roomId}`;
     ...
     */
    _initScene() {
        const smallAccountNode = window.Global.Tools.findNode(cc.director.getScene(), 'Canvas>DDZSmallAccount');
        if (smallAccountNode) {
        ...
        this._hidePlayerInfoList([0, 1, 2]);
        this._hideActionPrompt();
    },
     ...
     * @private
     */
    _hideActionButton: function () {
        for (var i = 0; i < this.jiaofenNodes.length; i++) {
            this.jiaofenNodes[i].active = false;
        }

        for (var i = 0; i < this.qiangdizhuNodes.length; i++) {
            this.qiangdizhuNodes[i].active = false;
        }

        for (var i = 0; i < this.chupaiNodes.length; i++) {
            this.chupaiNodes[i].active = false;
        }
    ...
    _hideClock: function () {
        for (var i = 0; i < this.clockNodes.length; i++) {
            this.clockNodes[i].active = false;
        }
    },
    ...
    _setThisPlayerSeat(playerList) {
        for (let i = 0; i < playerList.length; i += 1) {
            ...
            if (obj.player === this._userInfo.playerUuid) {
